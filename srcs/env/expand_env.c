...
/*   Updated: 2023/10/29 21:39:33 by atokamot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_dollar(t_list **token, t_list **head, t_list *env_list,
		t_list **prev, int *finish_status)
{
	char	*env_value;
	t_token	*tmp;
	t_list	*splited_env;
	...
	env_value = find_env_name(tmp->token_content, env_list, finish_status);
...
void	expand_env(t_list **token, t_list *env_list, int *finish_status)
{
	...
	while (head)
	{
		...
		if (tmp->status == TK_DOLL && is_heardoc(prev) == false)
			handle_dollar(token, &head, env_list, &prev, finish_status);
