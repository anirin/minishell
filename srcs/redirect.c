...
/*   Updated: 2023/09/08 00:24:43 by atsu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redirect_stdout(t_list *head)
{
	...
	while(head != NULL && (!(head->status == SPECIAL && ft_strncmp(head->content, "|", 1) == 0)))
	{
		...
		else if (head->status == SPECIAL && ft_strncmp(head->content, ">>", 3) == 0)
		{
			head = head->next;
			fd = open(head->content, O_WRONLY | O_APPEND | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
			dup2(fd, STDOUT_FILENO);
			close(fd);
		}
		else if (head->status == SPECIAL && ft_strncmp(head->content, ">>>", 4) == 0)
		{
...
void	redirect_stdin(t_list *head)
{
	...
	while(head != NULL && (!(head->status == SPECIAL && ft_strncmp(head->content, "|", 1) == 0)))
	{
		...
		{
			head = head->next;
			fd = open(".heardoc_tmp", O_RDWR | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO);
			while(line)
			{
